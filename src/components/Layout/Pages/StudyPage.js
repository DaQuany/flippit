...
const StudyPage = ({ selectedItems }) => {
  ...
  const renderLibraryImage = (option) => {
    switch (option) {
      case 'review':
        return <img src={ReviewImg} alt={`Library ${option}`} style={{ width: 200 }} />;
      case 'schedules':
        return <img src={SchedulesImg} alt={`Library ${option}`} style={{ width: 200 }} />;
      case 'reminder':
        return <img src={RemindersImg} alt={`Library ${option}`} style={{ width: 200 }} />;
      default:
        return null;
    }
  };
